		     ...
		     void **buf, size_t *size)
{
	...
	size_t ser_size = ((size_t)non_ser->size + 1) * sizeof(uint32_t);
	uint32_t *ser_buf = malloc(ser_size);
	if (!ser_buf)
		return VACCEL_ENOMEM;

	/* If the struct is empty skip element serialization */
	if (non_ser->array && non_ser->size) {
		ser_buf[0] = (int)non_ser->size;
		       ...
		       size_t data_size, void *out_data)
{
	...
	deser_buf->size = ser_buf[0];
	deser_buf->array = malloc(deser_buf->size * sizeof(uint32_t));
	if (!deser_buf->array)
		return VACCEL_ENOMEM;

	for (uint32_t i = 0; i < deser_buf->size; i++)
