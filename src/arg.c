...
/* Compact type descriptor for hybrid validation */
struct type_info {
...
int vaccel_arg_array_add_custom(struct vaccel_arg_array *array, void *buf,
				size_t size, uint32_t custom_id,
...
int vaccel_arg_array_get_serialized(struct vaccel_arg_array *array,
				    ...
				    uint32_t expected_custom_id,
				    size_t expected_size, void *data,
...
int vaccel_arg_array_set_custom(struct vaccel_arg_array *array, void *buf,
				size_t size, uint32_t expected_id,
				vaccel_arg_type_validator_fn validator)
{
	if (!array || !validator)
		return VACCEL_EINVAL;
...
size_t vaccel_arg_array_get_position(const struct vaccel_arg_array *array)
{
...
struct vaccel_arg *vaccel_arg_array_data(struct vaccel_arg_array *array)
{
