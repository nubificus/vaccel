...
int vaccel_blob_release(struct vaccel_blob *blob)
{
	...
	if (blob->data) {
		if (blob->type == VACCEL_BLOB_MAPPED && blob->size) {
 ...
 *
 * A file is initialized if either a path to the file
...
bool vaccel_blob_initialized(struct vaccel_blob *blob)
{
	...
	return ((blob->type == VACCEL_BLOB_FILE && blob->path) ||
		(blob->type == VACCEL_BLOB_BUFFER && blob->data &&
		 blob->size) ||
 ...
 * the file from which the `vaccel_blob` was created.
 */
