...
static int torch_model_destructor(void *data)
{
	struct vaccel_torch_saved_model *model =
		(struct vaccel_torch_saved_model *)data;
	if (!model)
...
{
	struct vaccel_torch_saved_model *new = calloc(1, sizeof(*new));
	if (!new)
...
int vaccel_torch_saved_model_set_path(
	...
	//vaccel_file_destroy(&model->model);
	printf("could not destroy because of TORCH\n");
...
{
	vaccel_debug("Registering new vAccel PyTorch model");
	...
	int ret = resource_new(res, VACCEL_RES_TORCH_MODEL, (void *)model,
			torch_model_destructor);
	if (ret)
		goto free_resource;
