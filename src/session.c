...
static int parse_plugin_spec(const char *spec, char **local_name,
			     char **remote_name)
{
	if (!spec || spec[0] == '\0' || !local_name || !remote_name)
		return VACCEL_EINVAL;
				    ...
				    const char *plugin_spec)
{
	...
	ret = parse_plugin_spec(plugin_spec, &plugin_name, &remote_name);
	if (ret) {
		vaccel_error("Invalid plugin spec '%s'", plugin_spec);
		return ret;
	}
	...
	/* No flag hints here; the plugin is selected by name */
