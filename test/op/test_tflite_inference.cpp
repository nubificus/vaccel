...
TEST_CASE("tensor_ops")
{
    ...
    // no data allocated
    struct vaccel_tflite_tensor* tensor1 =
        vaccel_tflite_tensor_allocate(2, dims, VACCEL_TFLITE_FLOAT32, 0);
    REQUIRE(tensor1);
    ...
    // set dummy data
    struct vaccel_tflite_tensor* tensor2 =
        vaccel_tflite_tensor_allocate(2, dims, VACCEL_TFLITE_FLOAT32, 0);
    REQUIRE(tensor2);
    REQUIRE(tensor2->data == nullptr);
    REQUIRE(tensor2->size == 0);

    ret = vaccel_tflite_tensor_set_data(tensor2, (void *)dims, sizeof(int32_t)*2);
    REQUIRE(ret == VACCEL_OK);
    ...
    struct vaccel_tflite_tensor* tensor3 =
        vaccel_tflite_tensor_allocate(2, nullptr, VACCEL_TFLITE_FLOAT32, 0);
    REQUIRE(tensor3);
    ...
    struct vaccel_tflite_tensor* tensor4 =
        vaccel_tflite_tensor_allocate(2, dims, VACCEL_TFLITE_FLOAT32, 8);
    REQUIRE(tensor4);
    ...
    // re-set dummy data
    ret = vaccel_tflite_tensor_set_data(tensor4, (void *)dims, sizeof(int32_t)*2);
    REQUIRE(ret == VACCEL_OK);
...
TEST_CASE("inference")
{
    ...
    int ret;
    const char* model_path = abs_path(SOURCE_ROOT, "examples/models/tf/lstm2.tflite");
    ...
    ret = vaccel_tflite_session_run(&vsess, &model, &in, 1, &out, 1, &status);
    REQUIRE(ret == VACCEL_OK);
